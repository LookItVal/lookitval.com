...
onMounted(() => {
  if (barbsPathLoaded.value && shaftPathLoaded.value) {
    ...
    animationTimeline.value = gsap.timeline({ paused: true });

    animationTimeline.value.to(barbsPathLoaded.value, {
      strokeDasharray: `${barbsLength} 0`,
      duration: 2,
      ease: props.easeFunction
    }, 0);

    animationTimeline.value.to(shaftPathLoaded.value, {
...
watch(() => props.progress, (newProgress) => {
  if (animationTimeline.value) {
    gsap.to(animationTimeline.value, {
